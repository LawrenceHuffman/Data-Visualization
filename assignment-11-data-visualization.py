...
def plot_category_performance_by_location():
    ...
    grouped_data = latest_data.pivot(index='Category', columns='Location', values='Sales')
...
def plot_ad_spend_vs_sales():
    ...
    top_outliers = sales_df.nlargest(2, 'Sales')
...
def plot_ad_efficiency_over_time():
    ...
    ax.annotate('Peak', xy=(efficiency.idxmax(), efficiency.max()),
...
def plot_purchase_by_age_group():
    ...
    data = [customer_df[customer_df['AgeGroup'] == group]['PurchaseAmount'] for group in age_labels]

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.boxplot(data, labels=age_labels)
...
def plot_category_market_share():
    ...
    fig, ax = plt.subplots(figsize=(8, 8))
    ax.pie(category_sales.values, labels=category_sales.index, autopct='%1.1f%%', explode=explode, startangle=90)
...
def create_business_dashboard():
    ...
    location_sales = sales_df.groupby('Location')['Sales'].sum()
    axes[0, 1].pie(location_sales.values, labels=location_sales.index, autopct='%1.1f%%', startangle=90)
    ...
    p = np.poly1d(z)
    x_line = np.linspace(sales_df['AdSpend'].min(), sales_df['AdSpend'].max(), 100)
    axes[1, 1].plot(x_line, p(x_line), linestyle='--', linewidth=2)
...
def main():
    ...
    print("Sales increase over time, especially in Q4.")
    print("Miami performs best overall, and Electronics is the top category.")
    print("Higher ad spend is generally linked to higher sales.")
    print("Most customers are middle-aged and buy mostly mid-range products.")
